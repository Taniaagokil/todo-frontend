        ...
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <div
              ...
              className={`group flex items-start sm:items-center justify-between p-4 rounded-xl border transition-all cursor-pointer select-none ${task.completed ? 'bg-success-10/20 border-success-20 hover:border-success-40' : 'bg-white border-gray-100 hover:border-primary-70/40'}`}
            >
              <div className="flex items-start sm:items-center gap-3.5 flex-1 pr-2">
                ...
                />
                <div>
                  <p className={`text-xs md:text-sm font-medium leading-relaxed transition-all ${task.completed ? 'line-through text-gray-40' : 'text-dark-70 group-hover:text-primary-100'}`}>
                    {task.title}
                  </p>
